...
export class RemoveRoleEnumFromUsers1758794113896 implements MigrationInterface {
    name = 'RemoveRoleEnumFromUsers1758794113896'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`offering_campaigns\` (\`id\` varchar(36) NOT NULL, \`name\` varchar(255) NOT NULL, \`description\` text NOT NULL, \`target_amount\` decimal(15,2) NULL, \`start_date\` date NOT NULL, \`end_date\` date NOT NULL, \`status\` enum ('draft', 'active', 'paused', 'completed', 'cancelled') NOT NULL DEFAULT 'draft', \`image_url\` varchar(500) NULL, \`settings\` json NULL, \`created_by_user_id\` varchar(36) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), INDEX \`idx_campaign_status\` (\`status\`), INDEX \`idx_campaign_dates\` (\`start_date\`, \`end_date\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`contribution_campaigns\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`description\` text NULL, \`start_date\` date NOT NULL, \`end_date\` date NULL, \`is_fixed_amount\` tinyint NOT NULL DEFAULT 0, \`fixed_amount\` decimal(10,2) NULL, \`status\` enum ('active', 'inactive', 'completed') NOT NULL DEFAULT 'active', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`created_by_user_id\` varchar(36) NULL, INDEX \`idx_campaign_status\` (\`status\`), UNIQUE INDEX \`IDX_eac55b2b0d8f51c082a01e643b\` (\`name\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`card_contributions\` (\`id\` int NOT NULL AUTO_INCREMENT, \`amount\` decimal(10,2) NOT NULL, \`contribution_method\` enum ('online', 'cash_on_site') NOT NULL, \`contribution_date\` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP, \`notes\` text NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`card_id\` int NULL, \`contributor_user_id\` varchar(36) NULL, \`payment_id\` int NULL, \`collected_by_user_id\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`contribution_cards\` (\`id\` int NOT NULL AUTO_INCREMENT, \`card_number\` varchar(50) NOT NULL, \`phone_number\` varchar(20) NULL, \`initial_amount\` decimal(10,2) NOT NULL DEFAULT '0.00', \`current_balance\` decimal(10,2) NOT NULL DEFAULT '0.00', \`qr_code_url\` varchar(255) NULL, \`is_physical\` tinyint NOT NULL DEFAULT 0, \`status\` enum ('active', 'inactive', 'completed', 'suspended') NOT NULL DEFAULT 'active', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`campaign_id\` int NULL, \`user_id\` varchar(36) NULL, INDEX \`idx_card_phone\` (\`phone_number\`), INDEX \`idx_card_campaign\` (\`campaign_id\`), INDEX \`idx_card_user\` (\`user_id\`), INDEX \`idx_card_number\` (\`card_number\`), UNIQUE INDEX \`IDX_d4564e835569b9cf738e71d033\` (\`card_number\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`card_contributions\` ADD CONSTRAINT \`FK_1d96dfa9b62fb4cd917e6ba2a85\` FOREIGN KEY (\`card_id\`) REFERENCES \`contribution_cards\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`card_contributions\` ADD CONSTRAINT \`FK_aa2a3ba90380a1e35b776c3d91d\` FOREIGN KEY (\`contributor_user_id\`) REFERENCES \`users\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`card_contributions\` ADD CONSTRAINT \`FK_50ce39b279167347f45e2827725\` FOREIGN KEY (\`payment_id\`) REFERENCES \`transactions\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`card_contributions\` ADD CONSTRAINT \`FK_06f8478e0a34bf1dd3a5a825e29\` FOREIGN KEY (\`collected_by_user_id\`) REFERENCES \`users\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`card_contributions\` DROP FOREIGN KEY \`FK_06f8478e0a34bf1dd3a5a825e29\``);
        await queryRunner.query(`ALTER TABLE \`card_contributions\` DROP FOREIGN KEY \`FK_50ce39b279167347f45e2827725\``);
        await queryRunner.query(`ALTER TABLE \`card_contributions\` DROP FOREIGN KEY \`FK_aa2a3ba90380a1e35b776c3d91d\``);
        await queryRunner.query(`ALTER TABLE \`card_contributions\` DROP FOREIGN KEY \`FK_1d96dfa9b62fb4cd917e6ba2a85\``);
        ...
        await queryRunner.query(`DROP TABLE \`card_contributions\``);
